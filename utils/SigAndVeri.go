...
func VerifyCryptEcc(srcStr, cryptStr string, PubKeyFile *ecdsa.PublicKey) (bool, error) {
	...
	//获取公钥验证数据
	//publicKey, err := GetPublicKeyByPemFile(PubKeyFile)
	if err != nil {
		return false, err
	}
	//使用公钥、原文、以及签名信息解密后的两个椭圆曲线的大整数参数进行校验
	verify := ecdsa.Verify(PubKeyFile, []byte(srcStr), &rint, &sint)
